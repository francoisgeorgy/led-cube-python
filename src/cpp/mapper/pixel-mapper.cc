...
//
//           ┌──────┬──────┬──────┐
// chain 1:  │0,0   │64,0  │128,0 │
//           │Left  │Front │Right │
//           └──────┴──────┴──────┘
//           ┌──────┬──────┬──────┐
// chain 2:  │0,64  │64,64 │128,64│
//           │Bottom│Back  │Top   │
//           └──────┴──────┴──────┘
//
// LOGICAL COORDINATES :
...
// └──────┘
//
class Cube2PMapperV0 : public PixelMapper {
public:
  Cube2PMapperV0() {}

  virtual const char *GetName() const { return "Cube2PV0"; }

  virtual bool SetParameters(int chain, int parallel, const char *param) {
    if ((parallel != 2) || (chain != 3)) {  // This mapper is hardcoded for a topology of 2 parallel chains of 3 panels each.
      fprintf(stderr, "Cube2PV0: need --led-parallel=2 and --led-chain=3 and \n");
      return false;
    }
    return true;
  }
...
//
/*
class CubeC1RC2BMapper : public PixelMapper {
public:
  CubeC1RC2BMapper() {}

  virtual const char *GetName() const { return "CubeC1RC2B"; }

  virtual bool SetParameters(int chain, int parallel, const char *param) {
    if ((parallel != 2) || (chain != 3)) {  // This mapper is hardcoded for a topology of 2 parallel chains of 3 panels each.
      fprintf(stderr, "CubeC1RC2B: need --led-parallel=2 and --led-chain=3 and \n");
      return false;
    }
    return true;
  }
...
//           └──────┴──────┴──────┘
//
class CubeMapper : public PixelMapper {
public:
  CubeMapper() {}
  ...
  virtual bool SetParameters(int chain, int parallel, const char *param) {
    if ((parallel != 2) || (chain != 3)) {  // This mapper is hardcoded for a topology of 2 parallel chains of 3 panels each.
      fprintf(stderr, "Cube: need --led-parallel=2 and --led-chain=3 and \n");
      return false;
    }
    return true;
  }
...
//  RegisterPixelMapperInternal(result, new CubeC1RC2BMapper());
  RegisterPixelMapperInternal(result, new CubeMapper());
  return result;
}
