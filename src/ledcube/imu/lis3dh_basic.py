...
Lib for the LIS3DH accelerometer.

Adapted from the Adafruit code.
...
class LIS3DH:
    ...
    def __init__(
        ...
        if device_id != 0x33:
            raise RuntimeError("Failed to find LIS3DH with ID 0x33. Check device ID.")
        # Reboot
        ...
        self.aligned = False
        self.offsets = AccelerationTuple(0, 0, 0)
    ...
    def get_offsets(self, v=[0.0, 0.0, 0.0], n=10) -> AccelerationTuple:
        x = 0
        y = 0
        z = 0
        for i in range(n):
            # gx, gy, gz = [value / adafruit_lis3dh.STANDARD_GRAVITY for value in lis3dh.acceleration]
            # gx, gy, gz = [value / STANDARD_GRAVITY for value in self.acceleration]
            gx, gy, gz = self.acceleration
            x += gx
            y += gy
            z += gz
        # accel_offsets = []
        # print("accel offset:", strvec(accel_offset))
        return AccelerationTuple(x/n - v[0], y/n - v[1], z/n - v[2])

    def align(self, v=[0.0, 0.0, 1.0]) -> None:
        self.offsets = self.get_offsets(v)
        self.aligned = True
    ...
    @use_offset.setter
    def use_offset(self, value) -> None:
        if value and not self.aligned:
            self.align()
